...
public class MapMain01 {
	...
	public static void main(String[] args) {
		...
		HashMap<Integer, String> map = new HashMap<>(); // new HashMap<Integer, String>(); 도 가능.
		...
		// keySet(): Map에서 key들로만 이루어진 집합(set)을 리턴.
		...
		// Map에 저장된 데이터를 삭제하는 메서드: remove(key)
		map.remove(101);
		System.out.println(map);
		
		// put(key, value):
		// (1) key가 존재하지 않으면, 새로운 key-value 쌍의 데이터를 저장.
		// (2) key가 존재하면, 해당 키(key)의 값을 변경.
		map.put(10, "No Name");
		System.out.println(map);
	}
