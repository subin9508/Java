 ...
 * |_이유: start() 호출 -> JRE에서 쓰레드를 초기화(메모리 할당, 스케쥴링, ...)  
...
public class ThreadMain01 {
	...
	public static void main(String[] args) {
		...
		class MyThread extends Thread {
			...
			public void run() {
				for(int i = 0; i < 100; i++) {
					...
					try {
						sleep(100); // Thread 클래스의 static 메서드 호출 - 출력 후, 0.1초(100ms = 100/1000초) 쉼.
		...
		// (3) MyThread 객체 생성
		 ...
		 long start = System.currentTimeMillis(); // 쓰레드 시작시간.
		 ...
		 long end = System.currentTimeMillis(); // th1, th2 모두 종료된 후 시간
