...
// class D extends A, C {} (X) // 다중 상속 - 자바는 클래스의 다중 상속을 허용하지 않는다.
...
interface K extends I, J {} // 인터페이스 K는 인터페이스 I와 J를 확장. 인터페이스는 다중 상속 가능. (개수 제한 없음)

// interface L extends A {} (X) // 인터페이스는 클래스를 확장할 수 없다. 인터페이스는 인터페이스만 확장 가능.

interface Clickable { // 대문자로 시작하고 -able로 끝나면 99% interface라고 볼 수 있음.
