...
//		}
		...
		List<Integer> odds2 = numbers.stream().filter((x) -> x % 2 == 1).map((x) -> x * x).toList();
		...
		List<String> result2 = languages.stream().filter((x) -> x.length() >= 5)
//				.map((x) -> x.toUpperCase())
		 ...
		 * 		x -> x.toUpperCase()
		 * 		String::toUpperCase ==> String 클래스가 가지고 있는 toUpperCase() 메서드를 참조하겠다는 의미.
		 * (2) 람다가 아규먼트를 1개만 갖고,
