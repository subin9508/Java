...
// 		Adder adder = new Adder();
		...
		//----- 람다 표현식 사용:
//		calc.calculate((double x, double y) -> { return x / y; } ); : 본래 람다 표현식
		 ...
		 * - 람다 표현식 몸통({})에 한 문장만 있는 경우, {}와 세미콜론 생략 가능.
		 * {}만 생략하면 안되고 {}와 세미콜론을 함께 생략해야 함.
		 * - 람다 표현식 몸통이 return 문장만 있는 경우에는 리턴 값만 남김. ('return'도 생략)
		 *  (x, y) -> x / y
		 *  - 람다 표현식에서 파라미터 개수가 1개인 경우에는 () 생략 가능.
		 *  (x) -> 2 * x ==> x -> 2 * x 식으로 표현 가능. (반드시 파라미터 개수가 1개인 경우에만!)
